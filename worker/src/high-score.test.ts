...
describe('GET', () => {
	...
	it('should return current entry', async () => {
		...
		expect(await response.json()).toEqual({ score: 0, name: 'Anonymous', timestamp: 0 });
	});
});
...
describe('POST', () => {
	...
	describe('when submitted score is not greater than existing score', () => {
		...
		it('should return a 200 status', async () => {
			request = await createRequest('POST', { score: 1, name: 'Not high enough' });
			response = await worker.fetch(request, env as Env);
			expect(response.status).toBe(200);
		});
		it('should return a success:false with error message if lower than existing score', async () => {
			...
			expect(await response.json()).toEqual({
				success: false,
				error: 'The score you submitted is not higher than the current high score of 2',
			});
		});
		it('should return a success:false with error message if equal to existing score', async () => {
			request = await createRequest('POST', { score: 2, name: 'Not high enough' });
			response = await worker.fetch(request, env as Env);
			expect(await response.json()).toEqual({
				success: false,
				error: 'The score you submitted is not higher than the current high score of 2',
